...
export async function POST(request: Request) {
  ...
  const document = await prisma.$transaction(async (tx) => {
    const created = await tx.studioDocument.create({ data: { title, slug, content, category, published: Boolean(body?.published) } });
    await tx.studioDocumentVersion.create({ data: { documentId: created.id, version: created.version, title: created.title, content: created.content, category: created.category } });
