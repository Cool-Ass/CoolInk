...
export async function POST(request: Request) {
  ...
  const serviceType = String(body?.serviceType ?? "tattoo").trim().slice(0, 80) || "tattoo";
  const workstation = String(body?.workstation ?? "").trim().slice(0, 80) || null;
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const conflict = await bookingConflict(startsAt, endsAt, undefined, !Boolean(body?.ignoreBuffer), tx, { serviceType, workstation });
      if (conflict.appointment || conflict.block) throw new BookingConflictError(bookingConflictMessage(conflict));
      ...
      const appointment = await tx.appointment.create({ data: { projectId: project.id, startsAt, endsAt, status: "confirmed", notes: String(body?.notes ?? "").trim().slice(0, 5_000) || null, price, serviceType, workstation } });
    ...
    if (error instanceof BookingConflictError) return NextResponse.json({ error: error.message || "Ten termin jest niedostępny. Wybierz inny zakres." }, { status: 409 });
