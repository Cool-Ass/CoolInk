...
export async function GET() {
  ...
  return NextResponse.json({ appointments: serialise(appointments), blocks: serialise(blocks), slots: serialise(slots), googleBusy: serialise(googleEvents), bufferMinutes: Math.min(240, Math.max(0, Number(bufferSetting?.value ?? 0) || 0)) });
}
