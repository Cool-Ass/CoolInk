...
export async function GET(request: Request) {
  ...
  const data = await prisma.client.findUnique({
    ...
    include: {
      ...
      notifications: true,
      directMessages: { select: { id: true, author: true, body: true, readAt: true, createdAt: true } },
  ...
  const acceptedDocumentVersions = data.acceptances.length
    ? await prisma.studioDocumentVersion.findMany({
        ...
        select: { documentId: true, version: true, title: true, content: true, formFields: true, category: true, createdAt: true },
      })
    : [];
