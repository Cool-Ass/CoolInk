...
export async function POST(request: Request) {
  ...
  if (!firstName || firstName.length > 80 || !lastName || lastName.length > 80 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || password.length < 10 || password.length > 128 || !privacyAcknowledged) return NextResponse.json({ error: "Podaj imię, nazwisko, poprawny e-mail i hasło oraz potwierdź zapoznanie się z polityką prywatności." }, { status: 400 });
