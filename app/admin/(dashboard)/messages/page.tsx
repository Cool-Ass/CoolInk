...
export default async function MessagesPage() {
  ...
  return <div className="flex flex-col gap-8">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-medium tracking-[0.2em] text-ink-gold">CENTRUM KOMUNIKACJI</p><h1 className="mt-2 font-display text-4xl text-ink-white">Wiadomości</h1><p className="mt-2 text-sm text-ink-grey">Jedna skrzynka dla wszystkich rozmów powiązanych z projektem.</p></div><MessageTemplateManager initial={templates} /></header>
    <AdminConversationInbox templates={templates} conversations={conversations.map((project) => ({ id: project.id, title: project.title, kind: project.kind, clientId: project.client.id, clientName: `${project.client.firstName} ${project.client.lastName}`, messages: project.messages.map((message) => ({ id: message.id, author: message.author, body: message.body, createdAt: message.createdAt.toISOString(), readAt: message.readAt?.toISOString() ?? null, attachment: message.attachment })) }))} />
