...
export default async function CalendarPage() {
  ...
  return <div className="flex flex-col gap-5">
    ...
    <CalendarHub stats={{ appointments: appointments.length, blocks: blocks.length, newProjects: projects.filter((project) => ["inquiry", "reviewing"].includes(project.status)).length }} appointments={appointments.map((appointment) => ({ id: appointment.id, startsAt: appointment.startsAt.toISOString(), endsAt: appointment.endsAt.toISOString(), status: appointment.status, price: appointment.price, notes: appointment.notes, clientId: appointment.project.clientId, clientName: `${appointment.project.client.firstName} ${appointment.project.client.lastName}`, projectTitle: appointment.project.title }))} blocks={blocks.map((block) => { const range = normalizeCalendarBlockRange(block); return { id: block.id, startsAt: range.startsAt.toISOString(), endsAt: range.endsAt.toISOString(), reason: block.reason }; })} slots={slots.map((item) => ({ id: item.id, startsAt: item.startsAt.toISOString(), endsAt: item.endsAt.toISOString(), title: item.title, description: item.description, color: item.color, icon: item.icon, isPublic: item.isPublic }))} promotions={promotions.map((item) => ({ id: item.id, title: item.title, description: item.description, badge: item.badge, startsAt: item.startsAt.toISOString(), endsAt: item.endsAt.toISOString(), color: item.color, icon: item.icon, isPublic: item.isPublic, active: item.active }))} events={events.map((item) => ({ id: item.id, title: item.title, description: item.description, startsAt: item.startsAt.toISOString(), endsAt: item.endsAt.toISOString(), allDay: item.allDay, color: item.color, icon: item.icon, label: item.label, isPublic: item.isPublic, google: Boolean(item.googleCalendarSync), syncStatus: item.googleCalendarSync?.syncStatus ?? null }))} bufferMinutes={Number(setting.get("booking_buffer_minutes")) || 30} visibleMonths={Math.min(12, Math.max(1, Number(setting.get("calendar_visible_months")) || 3))} defaultFreeStart={setting.get("calendar_default_free_start") || "10:00"} defaultFreeEnd={setting.get("calendar_default_free_end") || "18:00"} />
  </div>;
}
