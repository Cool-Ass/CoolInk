...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ access?: string }> }) {
  ...
  return <div className="flex flex-col gap-5">
    ...
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[11px] tracking-[0.18em] text-ink-gold">CENTRUM DOWODZENIA</p><h1 className="mt-1 font-display text-3xl sm:text-4xl">Co wymaga Twojej uwagi?</h1><p className="mt-1 text-sm text-ink-grey">Najważniejsze sprawy są ustawione według pilności.</p></div><div className="flex gap-2"><div className="min-w-24 border border-red-400/35 bg-red-500/5 px-3 py-2"><p className="text-[9px] tracking-[.1em] text-red-200">PILNE</p><p className="mt-0.5 font-display text-2xl">{actions.filter((item) => item.priority === 1).length}</p></div><div className="min-w-24 border border-ink-white/15 bg-ink-charcoal/25 px-3 py-2"><p className="text-[9px] tracking-[.1em] text-ink-grey">DO ZROBIENIA</p><p className="mt-0.5 font-display text-2xl">{actions.length}</p></div></div></header>

    <section className="border border-ink-white/10 bg-ink-charcoal/35 p-4"><div className="flex justify-between gap-4"><div><p className="text-[10px] tracking-widest text-ink-gold">DZISIEJSZE WIZYTY</p><h2 className="mt-1 font-display text-2xl">Plan dnia</h2></div><Link href="/admin/calendar" className="text-[10px] tracking-[.08em] text-ink-gold">KALENDARZ →</Link></div><div className="mt-3 divide-y divide-ink-white/10">{today.length ? today.map((item) => <Link key={item.id} href={`/admin/clients/${item.project.client.id}`} className="grid gap-2 py-3 text-sm sm:grid-cols-[90px_1fr_auto]"><p className="text-ink-gold">{item.startsAt.toLocaleTimeString("pl-PL", { hour: "2-digit", minute: "2-digit" })}–{item.endsAt.toLocaleTimeString("pl-PL", { hour: "2-digit", minute: "2-digit" })}</p><p><strong>{item.project.client.firstName} {item.project.client.lastName}</strong><span className="mt-0.5 block text-xs text-ink-grey">{item.project.kind === "consultation" ? "Konsultacja" : item.project.title} · {duration(item.startsAt, item.endsAt)}</span></p><StatusBadge status={item.status} /></Link>) : <p className="py-4 text-sm text-ink-grey">Brak wizyt na dziś.</p>}</div></section>
