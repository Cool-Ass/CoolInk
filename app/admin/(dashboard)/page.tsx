...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ access?: string }> }) {
  ...
  return <div className="flex flex-col gap-7">
    ...
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs tracking-[0.2em] text-ink-gold">CENTRUM DOWODZENIA</p><h1 className="mt-2 font-display text-4xl">Co wymaga Twojej uwagi?</h1><p className="mt-2 text-sm text-ink-grey">Najważniejsze sprawy są ustawione według pilności.</p></div><div className="flex gap-3"><div className="border border-red-400/35 px-4 py-3"><p className="text-xs text-red-200">PILNE</p><p className="mt-1 font-display text-2xl">{actions.filter((item) => item.priority === 1).length}</p></div><div className="border border-ink-white/15 px-4 py-3"><p className="text-xs text-ink-grey">DO ZROBIENIA</p><p className="mt-1 font-display text-2xl">{actions.length}</p></div></div></header>

    <section className="border border-ink-white/10 bg-ink-charcoal/40 p-5"><div className="flex justify-between gap-4"><div><p className="text-xs tracking-widest text-ink-gold">DZISIEJSZE WIZYTY</p><h2 className="mt-2 font-display text-2xl">Plan dnia</h2></div><Link href="/admin/calendar" className="text-xs text-ink-gold">KALENDARZ →</Link></div><div className="mt-4 divide-y divide-ink-white/10">{today.length ? today.map((item) => <Link key={item.id} href={`/admin/clients/${item.project.client.id}`} className="grid gap-2 py-4 text-sm sm:grid-cols-[90px_1fr_auto]"><p className="text-ink-gold">{item.startsAt.toLocaleTimeString("pl-PL", { hour: "2-digit", minute: "2-digit" })}–{item.endsAt.toLocaleTimeString("pl-PL", { hour: "2-digit", minute: "2-digit" })}</p><p><strong>{item.project.client.firstName} {item.project.client.lastName}</strong><span className="mt-1 block text-sm text-ink-grey">{item.project.kind === "consultation" ? "Konsultacja" : item.project.title} · {duration(item.startsAt, item.endsAt)}</span></p><StatusBadge status={item.status} /></Link>) : <p className="py-5 text-sm text-ink-grey">Brak wizyt na dziś.</p>}</div></section>

    <section className="border border-ink-white/10 bg-ink-charcoal/40 p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-xs tracking-widest text-ink-gold">KOLEJKA DZIAŁAŃ</p><h2 className="mt-2 font-display text-2xl">Następne kroki</h2></div><span className="text-sm text-ink-grey">{actions.length}</span></div><div className="mt-4 grid gap-3 xl:grid-cols-2">{actions.slice(0, 14).map((item) => <Link key={item.key} href={item.href} className={`border-l-4 p-4 transition-colors hover:bg-ink-white/5 ${item.priority === 1 ? "border-red-400 bg-red-500/5" : "border-ink-gold bg-ink-gold/5"}`}><div className="flex flex-wrap items-center justify-between gap-2"><p className={`text-xs tracking-[.1em] ${item.priority === 1 ? "text-red-200" : "text-ink-gold"}`}>{item.label}</p>{item.dueAt && <time className="text-xs text-ink-grey">{fmt(item.dueAt)}</time>}</div><p className="mt-2 text-sm text-ink-white">{item.title}</p><p className="mt-2 line-clamp-2 text-sm leading-relaxed text-ink-grey">{item.detail}</p></Link>)}{actions.length === 0 && <div className="border border-emerald-400/25 bg-emerald-400/5 p-5 text-sm text-emerald-200">Wszystko jest pod kontrolą.</div>}</div></section>

    <section className="border border-ink-white/10 bg-ink-charcoal/40 p-5"><p className="text-xs tracking-widest text-ink-gold">NAJBLIŻSZE 14 DNI</p><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{upcoming.map((item) => <Link key={item.id} href={`/admin/clients/${item.project.client.id}`} className="border border-ink-white/10 p-4 hover:border-ink-gold"><p className="text-sm">{item.project.client.firstName} {item.project.client.lastName}</p><p className="mt-1 text-sm text-ink-grey">{item.project.kind === "consultation" ? "Konsultacja" : item.project.title}</p><p className="mt-3 text-xs text-ink-gold">{fmt(item.startsAt)} · {duration(item.startsAt, item.endsAt)}</p></Link>)}{upcoming.length === 0 && <p className="text-sm text-ink-grey">Brak kolejnych wizyt w tym okresie.</p>}</div></section>
  </div>;
}
