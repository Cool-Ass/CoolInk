...
export default async function SettingsPage() {
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <section className="border border-ink-white/10 bg-ink-charcoal/30 p-6"><p className="text-xs tracking-[.14em] text-ink-gold">BEZPIECZEŃSTWO</p><h2 className="mt-2 font-display text-2xl">Dziennik działań administratorów</h2><p className="mt-2 text-sm text-ink-grey">Najważniejsze zmiany danych i ustawień są zapisywane wraz z kontem wykonującym operację.</p><div className="mt-5 divide-y divide-ink-white/10">{auditLogs.map((log) => <article key={log.id} className="grid gap-2 py-3 text-sm sm:grid-cols-[180px_1fr_auto]"><p className="text-ink-grey">{log.adminUser?.email || "Usunięte konto"}</p><p>{log.summary}</p><time className="text-xs text-ink-grey">{log.createdAt.toLocaleString("pl-PL", { dateStyle: "short", timeStyle: "short" })}</time></article>)}{auditLogs.length === 0 && <p className="py-4 text-sm text-ink-grey">Dziennik zacznie się wypełniać po kolejnych zmianach.</p>}</div></section>
