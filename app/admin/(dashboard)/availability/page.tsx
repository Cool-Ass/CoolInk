...
export default async function AvailabilityPage() {
  ...
  return <div className="flex flex-col gap-8"><div><p className="text-[11px] tracking-[0.16em] text-ink-gold">WIZYTY I KLIENCI</p><h1 className="mt-2 font-display text-3xl text-ink-white">Dostępność i promocje</h1><p className="mt-2 max-w-2xl text-sm text-ink-grey">Ustal swoje godziny pracy, zaplanuj dni wolne oraz przypnij promocję do wybranych terminów.</p></div><AvailabilityManager initialHours={hours.length ? hours : defaults} bufferMinutes={Number(buffer?.value) || 30} blocks={blocks.map((item) => ({ id: item.id, startsAt: item.startsAt.toISOString(), endsAt: item.endsAt.toISOString(), reason: item.reason }))} promotions={promotions.map((item) => ({ id: item.id, title: item.title, description: item.description, badge: item.badge, startsAt: item.startsAt.toISOString(), endsAt: item.endsAt.toISOString(), active: item.active }))} /></div>;
}
