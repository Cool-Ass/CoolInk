...
export function isGoogleCalendarResourceGone(error: unknown) {
...
export async function googleCalendarRequest<T>(accessToken: string, path: string, init?: RequestInit) {
  ...
  if (!response.ok) {
    const message = typeof data.error?.message === "string"
      ? data.error.message
      : "Błąd Google Calendar API.";
    const reasonValue = data.error?.errors?.[0]?.reason;
    throw new GoogleCalendarApiError(
      message,
      response.status,
      typeof reasonValue === "string" ? reasonValue : null,
    );
  }
  return data as T;
}
