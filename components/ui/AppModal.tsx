...
export default function AppModal({ title, subtitle, size = "md", onClose, children, footer, closeOnBackdrop = true, priority = false, headerAction }: { title: string; subtitle?: string; size?: ModalSize; onClose: () => void; children: ReactNode; footer?: ReactNode; closeOnBackdrop?: boolean; priority?: boolean; headerAction?: ReactNode }) {
  ...
  return <div className={`fixed inset-0 ${priority ? "z-[200]" : "z-[100]"} flex items-end justify-center bg-ink-black/80 p-0 backdrop-blur-sm sm:items-center sm:p-6`} onMouseDown={() => closeOnBackdrop && onClose()}>
    <section ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} onMouseDown={(event) => event.stopPropagation()} className={`h-[100dvh] w-full ${widths[size]} overscroll-contain overflow-y-auto border border-ink-white/15 bg-ink-charcoal shadow-2xl outline-none sm:h-auto sm:max-h-[88vh] sm:rounded-sm`}>
      <header className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-ink-white/10 bg-ink-charcoal/95 px-4 py-4 backdrop-blur sm:gap-5 sm:px-7"><div className="min-w-0"><p className="text-[10px] tracking-[0.16em] text-ink-gold">COOLINK</p><h2 id={titleId} className="mt-1 break-words font-display text-2xl text-ink-white sm:text-3xl">{title}</h2>{subtitle && <p className="mt-2 text-sm leading-relaxed text-ink-grey">{subtitle}</p>}</div><div className="flex shrink-0 items-center gap-2">{headerAction}<button ref={closeRef} type="button" onClick={onClose} aria-label="Zamknij" className="flex h-11 w-11 items-center justify-center border border-ink-white/15 text-xl text-ink-grey transition-colors hover:border-ink-gold hover:text-ink-gold">×</button></div></header>
      <div className="p-4 sm:p-7">{children}</div>
      {footer && <footer className="sticky bottom-0 border-t border-ink-white/10 bg-ink-charcoal/95 px-4 py-4 backdrop-blur sm:px-7">{footer}</footer>}
    </section>
  </div>;
}
