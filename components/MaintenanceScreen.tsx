...
import type { MaintenanceModuleData } from "@/lib/modules";
import { siteThemeStyle } from "@/lib/siteTheme";

export default function MaintenanceScreen({ content = DEFAULT_CONTENT.maintenance, theme = DEFAULT_CONTENT.theme, preview = false }: { content?: MaintenanceModuleData; theme?: SiteContent["theme"]; preview?: boolean }) {
  return (
    <main style={siteThemeStyle(theme)} className={`relative flex items-center justify-center overflow-hidden bg-ink-black px-4 py-10 text-ink-white sm:px-6 sm:py-12 ${preview ? "min-h-[720px]" : "min-h-[100svh]"}`}>
