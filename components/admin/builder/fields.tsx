...
const labelClass = "flex min-w-0 flex-col gap-1.5 text-[10px] tracking-[0.08em] text-white/65";
const inputClass = "min-h-11 w-full min-w-0 max-w-full border border-white/20 bg-[#17191c] px-2.5 text-[12px] normal-case tracking-normal text-white outline-none transition-colors hover:border-white/35 focus-visible:border-ink-gold focus-visible:ring-1 focus-visible:ring-ink-gold disabled:cursor-not-allowed disabled:opacity-45";
...
export function TextareaField({ label, value, onChange, rows = 4, placeholder }: { label: string; value: string; onChange: (v: string) => void; rows?: number; placeholder?: string }) {
  return <label className={labelClass}>{label}<textarea value={value} onChange={(event) => onChange(event.target.value)} rows={rows} placeholder={placeholder} className="min-h-24 w-full min-w-0 max-w-full resize-y border border-white/20 bg-[#17191c] px-2.5 py-2.5 text-[12px] leading-relaxed normal-case tracking-normal text-white outline-none transition-colors hover:border-white/35 focus-visible:border-ink-gold focus-visible:ring-1 focus-visible:ring-ink-gold" /></label>;
}
...
export function SelectField<T extends string>({ label, value, onChange, options }: { label: string; value: T; onChange: (v: T) => void; options: { value: T; label: string }[] }) {
  ...
  return <fieldset className="min-w-0"><legend className="sr-only">{label}</legend><div className="mb-1.5 flex min-w-0 items-center justify-between gap-2 text-[10px] tracking-[0.08em] text-white/65"><span className="shrink-0">{label}</span><span className="min-w-0 truncate normal-case tracking-normal text-white/75" title={selected}>{selected}</span></div><div className="grid min-w-0 gap-1" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(44px, 1fr))" }}>{options.map((option) => { const Icon = selectIcon(option.value); const active = option.value === value; return <button key={option.value} type="button" aria-pressed={active} aria-label={option.label} title={option.label} onClick={() => onChange(option.value)} className={`flex min-h-11 min-w-0 items-center justify-center gap-1 border px-1.5 text-[9px] leading-tight transition-colors active:bg-white/10 ${active ? "border-ink-gold bg-ink-gold/15 text-ink-gold" : "border-white/15 bg-[#17191c] text-white/60 hover:border-white/35 hover:text-white"}`}>{Icon ? <Icon aria-hidden className="h-4 w-4 shrink-0" /> : <span className="line-clamp-2 break-words">{option.label}</span>}</button>; })}</div></fieldset>;
}
...
export function ToggleField({ label, description, checked, onChange }: { label: string; description?: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return <label className="flex min-h-11 min-w-0 cursor-pointer items-start justify-between gap-3 border border-white/15 bg-[#17191c] px-3 py-2.5 transition-colors hover:border-white/30 focus-within:border-ink-gold focus-within:ring-1 focus-within:ring-ink-gold"><span className="min-w-0"><span className="block text-[10px] tracking-[0.06em] text-white">{label}</span>{description && <span className="mt-0.5 block text-[9px] leading-relaxed text-white/60">{description}</span>}</span><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="mt-0.5 h-5 w-5 shrink-0 accent-[#c99a4a]" /></label>;
}

export function FieldGroup({ title, children, defaultOpen = false }: { title: string; children: ReactNode; defaultOpen?: boolean }) {
  const [open, setOpen] = useState(defaultOpen);
  return <details open={open} onToggle={(event) => setOpen(event.currentTarget.open)} className="group min-w-0 border-t border-white/10"><summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 text-[11px] font-semibold tracking-[0.04em] text-white marker:hidden transition-colors hover:text-ink-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ink-gold">{title}<ChevronDown aria-hidden className="h-3.5 w-3.5 shrink-0 text-white/55 transition-transform group-open:rotate-180" /></summary><div className="flex min-w-0 flex-col gap-3 pb-4">{children}</div></details>;
}
...
export function BoxSpacingField({ label, value, onChange }: { label: string; value?: SpacingBox; onChange: (value: SpacingBox) => void }) {
  ...
  return <fieldset className="min-w-0"><div className="mb-1.5 flex items-center justify-between gap-2"><legend className="text-[10px] tracking-[0.08em] text-white/65">{label}</legend><span className="text-[9px] text-white/50">PX</span></div><div className="grid min-w-0 grid-cols-[repeat(4,minmax(0,1fr))_44px] gap-1">{sides.map((side, index) => <label key={side} className="min-w-0 text-center"><input aria-label={`${label}: ${labels[index]}`} type="number" value={current[side]} min={-300} max={600} onChange={(event) => change(side, Number(event.target.value) || 0)} className="h-11 w-full min-w-0 border border-white/20 bg-[#17191c] px-1 text-center text-[11px] text-white outline-none hover:border-white/35 focus-visible:border-ink-gold focus-visible:ring-1 focus-visible:ring-ink-gold" /><span className="mt-1 block truncate text-[8px] text-white/55">{labels[index]}</span></label>)}<button type="button" aria-pressed={linked} aria-label={linked ? "Rozłącz wartości" : "Połącz wartości"} title={linked ? "Rozłącz wartości" : "Połącz wartości"} onClick={() => setLinked((state) => !state)} className={`flex h-11 w-11 items-center justify-center border transition-colors ${linked ? "border-ink-gold bg-ink-gold/15 text-ink-gold" : "border-white/20 bg-[#25282c] text-white/60 hover:border-white/35 hover:text-white"}`}>{linked ? <Link2 aria-hidden className="h-4 w-4" /> : <Unlink aria-hidden className="h-4 w-4" />}</button></div></fieldset>;
}
