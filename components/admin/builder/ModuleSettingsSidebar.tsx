...
import { TextField, TextareaField, SelectField, NumberField, RangeField, ToggleField, FieldGroup, BoxSpacingField, PanelSection, DateTimeField } from "@/components/admin/builder/fields";
import { withDefaults, MODULE_LABELS, type Module, type ModuleStyle, type ResponsiveNumber } from "@/lib/modules";
...
export default function ModuleSettingsSidebar({ module, scope = "section", editorLabel, columnWidth, onColumnWidthChange, onChange, onStyleChange, onClose, portfolioItems, globalContact }: Props) {
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden bg-[#1d1f22]">
      <div className="flex h-10 shrink-0 items-center border-b border-white/10 px-1.5">
        <button type="button" onClick={onClose} aria-label="Wróć do elementów" title="Wróć do elementów" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md text-white/60 transition-colors hover:bg-white/5 hover:text-ink-gold"><ArrowLeft aria-hidden className="h-3.5 w-3.5" /></button>
        <p className="min-w-0 flex-1 truncate pr-9 text-center text-[11px] font-semibold text-white" title={editorLabel ?? `Edytuj: ${MODULE_LABELS[module.type]}`}>{editorLabel ?? `Edytuj: ${MODULE_LABELS[module.type]}`}</p>
      </div>

      <div role="tablist" aria-label="Zakładki ustawień elementu" className="grid shrink-0 grid-cols-3 border-b border-white/10">{tabs.map(({ id, label, icon: Icon }) => <button key={id} id={`builder-sidebar-tab-${id}`} type="button" role="tab" aria-selected={tab === id} aria-controls={`builder-sidebar-panel-${id}`} tabIndex={tab === id ? 0 : -1} onClick={() => setTab(id)} onKeyDown={(event) => { const index = tabs.findIndex((item) => item.id === id); const offset = event.key === "ArrowRight" ? 1 : event.key === "ArrowLeft" ? -1 : 0; const target = event.key === "Home" ? 0 : event.key === "End" ? tabs.length - 1 : offset ? (index + offset + tabs.length) % tabs.length : -1; if (target < 0) return; event.preventDefault(); const next = tabs[target].id; setTab(next); requestAnimationFrame(() => document.getElementById(`builder-sidebar-tab-${next}`)?.focus()); }} className={`flex min-h-11 min-w-0 flex-col items-center justify-center gap-0.5 border-b-2 px-1 text-[9px] transition-colors ${tab === id ? "border-ink-gold bg-ink-gold/5 text-white" : "border-transparent text-white/55 hover:bg-white/[0.035] hover:text-white"}`}><Icon aria-hidden className="h-3.5 w-3.5" /><span className="truncate">{label}</span></button>)}</div>
      <div data-lenis-prevent role="tabpanel" id={`builder-sidebar-panel-${tab}`} aria-labelledby={`builder-sidebar-tab-${tab}`} className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain p-2.5 [scrollbar-gutter:stable]">
...
interface SavedStylePreset { id: string; name: string; style: ModuleStyle }
const STYLE_PRESETS_KEY = "coolink-builder-style-presets-v1";

function StylePresetControls({ value, onChange }: { value: ModuleStyle; onChange: (style: ModuleStyle) => void }) {
  const [presets, setPresets] = useState<SavedStylePreset[]>([]);
  const [selected, setSelected] = useState("");
  useEffect(() => {
    let cancelled = false;
    try {
      const parsed = JSON.parse(window.localStorage.getItem(STYLE_PRESETS_KEY) ?? "[]");
      if (Array.isArray(parsed)) queueMicrotask(() => { if (!cancelled) setPresets(parsed.slice(0, 50)); });
    } catch { /* optional browser library */ }
    return () => { cancelled = true; };
  }, []);
  const persist = (next: SavedStylePreset[]) => {
    setPresets(next);
    try { window.localStorage.setItem(STYLE_PRESETS_KEY, JSON.stringify(next)); } catch { /* optional browser library */ }
  };
  return <PanelSection title="Wielorazowe style">
    <SelectField label="ZAPISANY STYL" value={selected} onChange={(id) => setSelected(id)} options={[{ value: "", label: "Wybierz preset" }, ...presets.map((preset) => ({ value: preset.id, label: preset.name }))]} />
    <div className="grid grid-cols-3 gap-1.5">
      <button type="button" onClick={() => { const name = window.prompt("Nazwa stylu"); if (!name?.trim()) return; const preset = { id: crypto.randomUUID(), name: name.trim().slice(0, 60), style: structuredClone(value) }; persist([...presets, preset].slice(-50)); setSelected(preset.id); }} className="min-h-9 border border-white/15 px-2 text-[9px] text-white/65 hover:border-ink-gold hover:text-ink-gold">ZAPISZ</button>
      <button type="button" disabled={!selected} onClick={() => { const preset = presets.find((item) => item.id === selected); if (preset) onChange(structuredClone(preset.style)); }} className="min-h-9 border border-white/15 px-2 text-[9px] text-white/65 hover:border-ink-gold hover:text-ink-gold disabled:opacity-30">ZASTOSUJ</button>
      <button type="button" disabled={!selected} onClick={() => { persist(presets.filter((item) => item.id !== selected)); setSelected(""); }} className="min-h-9 border border-red-400/30 px-2 text-[9px] text-red-300 disabled:opacity-30">USUŃ</button>
    </div>
  </PanelSection>;
}

function ResponsiveNumberFields({ label, value, fallback = 0, min = -4_000, max = 4_000, onChange }: { label: string; value?: ResponsiveNumber; fallback?: number; min?: number; max?: number; onChange: (value: ResponsiveNumber) => void }) {
  ...
  return <fieldset><legend className="mb-1.5 text-[10px] tracking-[.08em] text-ink-grey">{label}</legend><div className="grid grid-cols-3 gap-1.5">{(["desktop", "tablet", "mobile"] as const).map((key) => <NumberField key={key} label={key === "desktop" ? "KOMPUTER" : key === "tablet" ? "TABLET" : "TELEFON"} value={current[key]} min={min} max={max} onChange={(next) => onChange({ ...current, [key]: next })} />)}</div></fieldset>;
}
...
function AdvancedControls({ value, onChange }: { value?: ModuleStyle; onChange: (style: ModuleStyle) => void }) {
  ...
  return <div className="flex flex-col">
    ...
    <PanelSection title="Rozmiar, uchwyty i proporcje"><SelectField label="PRESET SZEROKOŚCI TREŚCI" value={style.contentWidth ?? "full"} onChange={(contentWidth) => set({ contentWidth })} options={[{ value: "full", label: "Pełna szerokość" }, { value: "wide", label: "Szeroka" }, { value: "normal", label: "Standardowa" }, { value: "narrow", label: "Wąska" }]} /><ResponsiveNumberFields label="SZEROKOŚĆ RESPONSYWNA (PX)" value={style.responsiveWidth} fallback={style.width ?? 0} min={0} onChange={(responsiveWidth) => set({ responsiveWidth, width: responsiveWidth.desktop || undefined })} /><ResponsiveNumberFields label="WYSOKOŚĆ RESPONSYWNA (PX)" value={style.responsiveHeight} fallback={style.height ?? 0} min={0} onChange={(responsiveHeight) => set({ responsiveHeight, height: responsiveHeight.desktop || undefined })} /><div className="grid grid-cols-2 gap-2"><NumberField label="MIN. SZEROKOŚĆ" value={style.minWidth ?? 0} min={0} max={4000} onChange={(minWidth) => set({ minWidth: minWidth || undefined })} /><NumberField label="MAKS. SZEROKOŚĆ" value={style.maxWidth ?? 0} min={0} max={4000} onChange={(maxWidth) => set({ maxWidth: maxWidth || undefined })} /><NumberField label="MIN. WYSOKOŚĆ" value={style.minHeight ?? 0} min={0} max={4000} onChange={(minHeight) => set({ minHeight: minHeight || undefined })} /><NumberField label="MAKS. WYSOKOŚĆ" value={style.maxHeight ?? 0} min={0} max={4000} onChange={(maxHeight) => set({ maxHeight: maxHeight || undefined })} /><NumberField label="SNAP / SIATKA PX" value={style.resizeSnap ?? 8} min={0} max={100} onChange={(resizeSnap) => set({ resizeSnap })} /><SelectField label="BLOKADA OSI" value={style.resizeAxis ?? "both"} onChange={(resizeAxis) => set({ resizeAxis })} options={[{ value: "both", label: "Obie osie" }, { value: "horizontal", label: "Tylko poziomo" }, { value: "vertical", label: "Tylko pionowo" }]} /></div><TextField label="PROPORCJE CSS" value={style.aspectRatio ?? ""} onChange={(aspectRatio) => set({ aspectRatio })} placeholder="np. 16 / 9 albo 1 / 1" /><ToggleField label="ZABLOKUJ PROPORCJE" description="Uchwyty narożne zachowują proporcje elementu. Shift włącza blokadę tymczasowo." checked={Boolean(style.lockAspectRatio)} onChange={(lockAspectRatio) => set({ lockAspectRatio })} /></PanelSection>
    ...
    <PanelSection title="Timeline animacji"><SelectField label="EFEKT" value={style.animation ?? "none"} onChange={(animation) => set({ animation })} options={[{ value: "none", label: "Brak" }, { value: "fade", label: "Pojawienie" }, { value: "fade-up", label: "Z dołu" }, { value: "fade-down", label: "Z góry" }, { value: "fade-left", label: "Z prawej" }, { value: "fade-right", label: "Z lewej" }, { value: "zoom", label: "Zoom" }, { value: "flip", label: "Obrót 3D" }, { value: "rotate", label: "Obrót" }, { value: "blur-in", label: "Wyostrzenie" }, { value: "bounce", label: "Podskok" }, { value: "float", label: "Lewitacja" }, { value: "pulse", label: "Puls" }]} />{style.animation && style.animation !== "none" && <><div className="grid grid-cols-2 gap-2"><SelectField label="TRIGGER" value={style.animationTrigger ?? "load"} onChange={(animationTrigger) => set({ animationTrigger })} options={[{ value: "load", label: "Załadowanie" }, { value: "view", label: "Wejście w ekran" }, { value: "hover", label: "Najechanie" }]} /><SelectField label="KRZYWA" value={style.animationEasing ?? "ease-out"} onChange={(animationEasing) => set({ animationEasing })} options={[{ value: "ease", label: "Płynna" }, { value: "ease-in", label: "Przyspiesza" }, { value: "ease-out", label: "Zwalnia" }, { value: "ease-in-out", label: "Obustronna" }, { value: "linear", label: "Liniowa" }, { value: "spring", label: "Sprężysta" }]} /><NumberField label="CZAS MS" value={style.animationDuration ?? 700} min={50} max={10000} step={50} onChange={(animationDuration) => set({ animationDuration })} /><NumberField label="OPÓŹNIENIE MS" value={style.animationDelay ?? 0} min={0} max={10000} step={50} onChange={(animationDelay) => set({ animationDelay })} /><NumberField label="POWTÓRZENIA" value={style.animationRepeat ?? 1} min={1} max={50} onChange={(animationRepeat) => set({ animationRepeat, animationIteration: "once" })} /><SelectField label="PĘTLA" value={style.animationIteration ?? "once"} onChange={(animationIteration) => set({ animationIteration })} options={[{ value: "once", label: "Ograniczona" }, { value: "infinite", label: "Nieskończona" }]} /><NumberField label="STAGGER MS" value={style.animationStagger ?? 0} min={0} max={5000} step={25} onChange={(animationStagger) => set({ animationStagger })} /><NumberField label="KOLEJNOŚĆ" value={style.animationOrder ?? 0} min={0} max={100} onChange={(animationOrder) => set({ animationOrder })} /></div><p className="text-[9px] leading-relaxed text-ink-grey">Łączne opóźnienie = opóźnienie + stagger × kolejność.</p></>}</PanelSection>
    <PanelSection title="Maska i dzielniki"><SelectField label="MASKA / KSZTAŁT" value={style.maskShape ?? "none"} onChange={(maskShape) => set({ maskShape })} options={[{ value: "none", label: "Brak" }, { value: "slant-left", label: "Skos w lewo" }, { value: "slant-right", label: "Skos w prawo" }, { value: "diamond", label: "Diament" }, { value: "hexagon", label: "Sześciokąt" }, { value: "circle", label: "Koło" }, { value: "ellipse", label: "Elipsa" }, { value: "ticket", label: "Bilet" }, { value: "arch", label: "Łuk" }]} /><ImageUploadField label="WŁASNA MASKA PNG / SVG" value={style.maskImage ?? ""} onChange={(maskImage) => set({ maskImage })} /><div className="grid grid-cols-2 gap-2"><SelectField label="DZIELNIK GÓRA" value={style.shapeDividerTop ?? "none"} onChange={(shapeDividerTop) => set({ shapeDividerTop })} options={[{ value: "none", label: "Brak" }, { value: "slant", label: "Skos" }, { value: "wave", label: "Fala" }, { value: "zigzag", label: "Zygzak" }, { value: "curve", label: "Krzywa" }]} /><SelectField label="DZIELNIK DÓŁ" value={style.shapeDividerBottom ?? "none"} onChange={(shapeDividerBottom) => set({ shapeDividerBottom })} options={[{ value: "none", label: "Brak" }, { value: "slant", label: "Skos" }, { value: "wave", label: "Fala" }, { value: "zigzag", label: "Zygzak" }, { value: "curve", label: "Krzywa" }]} /></div><ColorPicker label="KOLOR DZIELNIKA" value={style.shapeDividerColor ?? ""} onChange={(shapeDividerColor) => set({ shapeDividerColor })} /><RangeField label="WYSOKOŚĆ DZIELNIKA" value={style.shapeDividerHeight ?? 48} min={4} max={240} suffix="px" onChange={(shapeDividerHeight) => set({ shapeDividerHeight })} /></PanelSection>
    <PanelSection title="Widoczność i akcje"><SelectField label="TRYB PUBLICZNY" value={style.interactionMode ?? "normal"} onChange={(interactionMode) => set({ interactionMode })} options={[{ value: "normal", label: "Zwykły element" }, { value: "modal", label: "Otwieraj w modalu" }, { value: "drawer", label: "Otwieraj w panelu bocznym" }]} />{style.interactionMode && style.interactionMode !== "normal" && <TextField label="TEKST PRZYCISKU" value={style.interactionLabel ?? ""} onChange={(interactionLabel) => set({ interactionLabel })} placeholder="np. Zobacz szczegóły" />}<div className="grid grid-cols-2 gap-2"><DateTimeField label="POKAŻ OD" value={style.visibleFrom ?? ""} onChange={(visibleFrom) => set({ visibleFrom: visibleFrom || undefined })} /><DateTimeField label="POKAŻ DO" value={style.visibleUntil ?? ""} onChange={(visibleUntil) => set({ visibleUntil: visibleUntil || undefined })} /></div><ToggleField label="POSTĘP PRZEWIJANIA" description="Cienka belka pokazuje postęp czytania elementu." checked={Boolean(style.scrollProgress)} onChange={(scrollProgress) => set({ scrollProgress })} /><SelectField label="TOKEN WYGLĄDU" value={style.designToken ?? "none"} onChange={(designToken) => set({ designToken })} options={[{ value: "none", label: "Bez tokenu" }, { value: "surface", label: "Powierzchnia" }, { value: "accent", label: "Akcent" }, { value: "muted", label: "Wyciszony" }]} /></PanelSection>
...
function renderFields(
  ...
  switch (module.type) {
    case "siteHeader": {
      ...
      return <>
        <FieldGroup title="MARKA"><ImageUploadField label="Logo" value={d.logoUrl} onChange={(logoUrl) => onChange({ ...d, logoUrl })} previewFit="contain" transparentPreview helpText="Obsługiwane są przezroczyste pliki PNG, WebP i SVG." /><TextField label="Opis logo" value={d.logoAlt} onChange={(logoAlt) => onChange({ ...d, logoAlt })} /><TextField label="Nazwa awaryjna" value={d.brandName} onChange={(brandName) => onChange({ ...d, brandName })} /></FieldGroup>
    ...
    case "innerSection": {
      ...
      return <>
        <p className="rounded-r-md border-l-2 border-ink-gold/70 bg-ink-gold/5 px-2 py-1.5 text-[9px] leading-relaxed text-ink-grey">Przeciągnij widget do kolumny. Kliknij element na podglądzie, aby go edytować.</p>
        ...
        <SelectField label="Odstęp góra / dół" value={d.padding} onChange={(v) => onChange({ ...d, padding: v })} options={[{ value: "none", label: "Bez odstępu" }, { value: "sm", label: "Mały" }, { value: "md", label: "Średni" }, { value: "lg", label: "Duży" }]} />
      </>;
    }
    case "faq": {
    ...
    case "beforeAfter": { const d = withDefaults("beforeAfter", module.data); return <><ImageUploadField label="Zdjęcie przed" value={d.beforeImage} onChange={(beforeImage) => onChange({ ...d, beforeImage })} /><ImageUploadField label="Zdjęcie po" value={d.afterImage} onChange={(afterImage) => onChange({ ...d, afterImage })} /><div className="grid grid-cols-2 gap-2"><TextField label="Etykieta przed" value={d.beforeLabel} onChange={(beforeLabel) => onChange({ ...d, beforeLabel })} /><TextField label="Etykieta po" value={d.afterLabel} onChange={(afterLabel) => onChange({ ...d, afterLabel })} /></div><RangeField label="POCZĄTKOWA POZYCJA" value={d.position} min={5} max={95} suffix="%" onChange={(position) => onChange({ ...d, position })} /><SelectField label="PROPORCJE" value={d.aspect} onChange={(aspect) => onChange({ ...d, aspect })} options={[{ value: "landscape", label: "Poziome" }, { value: "square", label: "Kwadrat" }, { value: "portrait", label: "Pionowe" }]} /></>; }
    case "countdown": { const d = withDefaults("countdown", module.data); return <><TextField label="Nadpis" value={d.eyebrow} onChange={(eyebrow) => onChange({ ...d, eyebrow })} /><TextField label="Tytuł" value={d.title} onChange={(title) => onChange({ ...d, title })} /><DateTimeField label="DATA I GODZINA" value={d.targetDate} onChange={(targetDate) => onChange({ ...d, targetDate })} /><TextField label="Tekst po zakończeniu" value={d.expiredText} onChange={(expiredText) => onChange({ ...d, expiredText })} /><TextField label="Tekst przycisku" value={d.buttonLabel} onChange={(buttonLabel) => onChange({ ...d, buttonLabel })} /><TextField label="Link przycisku" value={d.href} onChange={(href) => onChange({ ...d, href })} /></>; }
    case "clientAuthForm":
    case "adminLoginForm":
      return <p className="border-l-2 border-ink-gold bg-ink-gold/5 px-3 py-3 text-[11px] leading-relaxed text-ink-grey">To bezpieczny widget systemowy. Jego pola są stałe, ale możesz zmieniać położenie, rozmiar, tło, odstępy i zachowanie responsywne.</p>;
