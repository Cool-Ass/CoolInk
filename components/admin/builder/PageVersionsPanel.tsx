  ...
  return (
    <aside data-lenis-prevent aria-label="Wersje opublikowane strony" onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); onClose(); } }} className="builder-floating-panel fixed bottom-2 right-2 top-44 z-[140] flex min-h-0 w-[min(22rem,calc(100vw-1rem))] flex-col overflow-hidden border border-white/20 bg-[#151618]/[0.98] shadow-2xl shadow-black/70 backdrop-blur-xl sm:bottom-4 sm:right-4 sm:top-36 sm:w-[min(22rem,calc(100vw-2rem))] lg:top-28 2xl:top-20">
      <header className="flex shrink-0 items-start justify-between gap-3 border-b border-white/10 px-4 py-3">
        ...
        <button type="button" onClick={onClose} aria-label="Zamknij wersje" className="flex h-11 w-11 shrink-0 items-center justify-center border border-white/20 text-white/60 hover:border-ink-gold hover:text-ink-gold"><X aria-hidden className="h-4 w-4" /></button>
      </header>
      ...
      <p className="shrink-0 border-t border-white/10 px-4 py-3 text-[11px] leading-relaxed text-white/60">{restoreBlocked ? "Poczekaj na autozapis bieżących zmian." : "Przywrócenie zmienia tylko szkic. Strona publiczna pozostaje bez zmian do ponownej publikacji."}</p>
    </aside>
  );
}
