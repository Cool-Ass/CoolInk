...
export default function IconPicker({ value, onChange, label = "IKONA" }: { value?: string; onChange: (value: string) => void; label?: string }) {
  ...
  return <div className="flex min-w-0 flex-col gap-2"><label className="flex min-w-0 flex-col gap-1.5 text-[11px] tracking-[0.1em] text-white/65">{label}<input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Szukaj ikony…" className="h-9 rounded-md w-full min-w-0 border border-white/20 bg-[#17191c] px-3 text-[11px] normal-case tracking-normal text-white outline-none hover:border-white/35 focus-visible:border-ink-gold focus-visible:ring-1 focus-visible:ring-ink-gold" /></label><div role="group" aria-label={`Lista ikon: ${label}`} className="grid max-h-40 min-w-0 grid-cols-6 gap-1.5 overflow-y-auto overscroll-contain border border-white/10 p-2 [scrollbar-gutter:stable]">{icons.map(([name, Icon]) => { const active = selected === name; return <button key={name} type="button" title={name} aria-label={`Wybierz ikonę ${name}`} aria-pressed={active} onClick={() => onChange(active ? "" : name)} className={`flex h-8 rounded-md min-h-8 min-w-0 items-center justify-center border transition-colors active:bg-white/10 ${active ? "border-ink-gold bg-ink-gold/15 text-ink-gold" : "border-white/15 text-white/60 hover:border-ink-gold hover:text-ink-gold"}`}><Icon aria-hidden className="h-4 w-4" /></button>; })}</div>{icons.length === 0 && <p role="status" className="text-[11px] text-white/55">Brak pasujących ikon.</p>}<button type="button" disabled={!selected} onClick={() => onChange("")} className="min-h-8 rounded-md self-start border border-white/15 px-3 text-[11px] text-white/60 hover:border-white/35 hover:text-white disabled:opacity-40">Wyczyść ikonę</button></div>;
}
