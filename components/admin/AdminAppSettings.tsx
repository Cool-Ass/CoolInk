...
export default function AdminAppSettings() {
  ...
  return <CollapsibleAdminSection title="APLIKACJA ADMINA" summary={enabled ? "Push włączony na tym urządzeniu" : installed ? "Panel zainstalowany" : "Instalacja i powiadomienia push"} icon={<Smartphone className="h-5 w-5" />} storageKey="admin:app-settings"><p className="max-w-2xl text-sm leading-relaxed text-ink-grey">Zainstaluj panel jako osobną aplikację i otrzymuj powiadomienia, gdy klient wyśle wiadomość, doda inspirację, zarezerwuje lub anuluje termin.</p><div className="mt-5 flex flex-wrap gap-2">{installPrompt && !installed && <button type="button" onClick={install} className="inline-flex items-center gap-2 border border-ink-gold bg-ink-gold px-4 py-3 text-xs text-ink-black"><Download className="h-4 w-4" />ZAINSTALUJ PANEL</button>}{installed && <span className="inline-flex items-center gap-2 border border-emerald-400/40 px-4 py-3 text-xs text-emerald-300">✓ PANEL ZAINSTALOWANY</span>}{supported ? <button type="button" disabled={busy} onClick={togglePush} className={`inline-flex items-center gap-2 border px-4 py-3 text-xs ${enabled ? "border-red-400/50 text-red-200" : "border-ink-gold text-ink-gold"}`}><BellRing className="h-4 w-4" />{busy ? "CHWILA…" : enabled ? "WYŁĄCZ PUSH" : "WŁĄCZ PUSH OD KLIENTÓW"}</button> : <span className="text-xs text-ink-grey">Ta przeglądarka nie obsługuje push.</span>}</div>{!installPrompt && !installed && <p className="mt-4 text-xs text-ink-grey">iPhone/iPad: Safari → Udostępnij → „Do ekranu początkowego”. Na komputerze opcja instalacji jest dostępna w pasku adresu.</p>}{message && <p role="status" className="mt-3 text-xs text-ink-grey">{message}</p>}</CollapsibleAdminSection>;
}
