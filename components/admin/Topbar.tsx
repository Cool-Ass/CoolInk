...
export default function Topbar({
  ...
  return <header className="relative flex items-center justify-between border-b border-ink-white/10 bg-ink-black/60 px-4 py-4 sm:px-6 md:px-10">
    <div className="flex min-w-0 items-center gap-3">
      ...
      <p className="truncate text-sm tracking-[0.04em] text-ink-grey"><span className="md:hidden">PANEL ADMINA</span><span className="hidden md:inline">{ADMIN_ROLE_LABEL[normalizeAdminRole(adminRole)]} · <span className="text-ink-white">{adminEmail}</span></span></p>
    </div>
    <div className="flex items-center gap-2">
      <button type="button" onClick={() => setSearchOpen(true)} aria-label="Szukaj w systemie" title="Szukaj (Ctrl+K)" className="flex h-10 items-center gap-2 border border-ink-white/20 px-3 text-ink-grey hover:border-ink-gold hover:text-ink-gold"><Search className="h-4 w-4" /><span className="hidden text-xs lg:inline">SZUKAJ</span></button>
