...
type Profile = { firstName: string; lastName: string; phone: string | null; email?: string; avatarUrl?: string | null };

export default function ProfileSettings({ client }: { client: Profile }) {
  ...
  const avatarSource = imageSource(value.avatarUrl);

  async function save() {
    setBusy(true); setMessage("");
    try {
      const response = await fetch("/api/client/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(value) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error);
      setMessage("Dane zapisane."); router.refresh();
  ...
  async function requestDeletion() {
    setBusy(true);
    try {
      const response = await fetch("/api/client/profile", { method: "DELETE" });
      if (!response.ok) throw new Error("Nie udało się wysłać zgłoszenia.");
      router.push("/app?deletion=requested"); router.refresh();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Nie udało się wysłać zgłoszenia."); setConfirmDelete(false); }
    finally { setBusy(false); }
  }
  ...
  return <section id="profil" className="scroll-mt-6 border border-ink-white/10 bg-ink-charcoal/30 p-4">
    ...
    <div className="mt-3 flex flex-wrap gap-3"><Link href="/polityka-prywatnosci" className="text-[10px] text-ink-gold underline">Prywatność i RODO</Link><a href="/api/client/data-export" download className="text-[10px] text-ink-gold underline">Pobierz kopię swoich danych</a></div>
    ...
    {confirmDelete && <ConfirmModal message="Wysłać prośbę o usunięcie konta? Studio zweryfikuje obowiązkowy okres przechowywania dokumentacji i potwierdzi wykonanie. Zostaniesz teraz wylogowany(-a)." onCancel={() => setConfirmDelete(false)} onConfirm={requestDeletion} pending={busy} pendingLabel="WYSYŁANIE…" />}
  </section>;
}
