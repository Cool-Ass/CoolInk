...
const nextConfig: NextConfig = {
  async headers() {
    ...
    const csp = `default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self'; img-src 'self' data: blob: https:; style-src 'self' 'unsafe-inline'; ${scriptSource}; connect-src 'self' https://*.supabase.co; font-src 'self' data:; frame-src https://accounts.google.com https://www.google.com https://maps.google.com https://www.youtube-nocookie.com https://player.vimeo.com${process.env.NODE_ENV === "production" ? "; upgrade-insecure-requests" : ""}`;
    return [{
      source: "/(.*)",
      headers: [
